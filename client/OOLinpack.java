/*
Modified by Thomas Keane Fri Oct 29 14:11:51 BST 2004 @591 /Internet Time/
- removed the GUI elements of the program so can be run from the command line
- psize is passed in as a parameter (not sure what effect other values will have on the measure so recommended to use 500)
...
public class OOLinpack
{
	...
	private double second_orig;

	public OOLinpack( int size )
	{
		psize = size;
		mflops_result = 0.0;
		residn_result = 0.0;
	...
	private double matgen (double a[][], int lda, int n, double b[])
	{
		Random gen;
		double norma;
		int init, i, j;
	...
	on entry a       double precision[n][lda]
  ...
  {
	  OOLinpack oo = new OOLinpack( 500 );
	  System.out.println( oo.getMFLOPS() );
  }
}
